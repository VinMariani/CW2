...
class Git(VersionControl):
    ...
    def get_revision_sha(cls, dest, rev):
        ...
        tag_ref = f'refs/tags_in_posts/{rev}'
    ...
    def _should_fetch(cls, dest, rev):
        ...
        Branches and tags_in_posts are not considered in this method because they are
        assumed to be always available locally (which is a normal outcome of
        ``git clone`` and ``git fetch --tags_in_posts``).
        """
    ...
    def update(self, dest, url, rev_options):
        ...
        if self.get_git_version() >= (1, 9):
            # fetch tags_in_posts in addition to everything else
            self.run_command(['fetch', '-q', '--tags_in_posts'], cwd=dest)
        else:
